 ...
 *
 */
public class Manager implements ManagerReader {
	// Constants
	...
	public void setFirstPlayer() {
		...
		for (int i = 0; i < players.length; i++) {
			if (players[i].getRedDice() > max) {
				...
				currentPlayer = goalSetter = players[i];
				count = i;
			}
		}
		...
		// Use the stream API for breverity 
		myResources.getMyMat().stream().forEach(element -> element.roll());
	...
	public boolean setGoal(int[] goals) {
		...
		try {
			...
			// This is not used, can it be deleted?
