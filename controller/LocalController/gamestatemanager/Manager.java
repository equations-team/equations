...
import localhostgame.GameTimer;
import localhostgame.Goal;
import localhostgame.Mat;
import Solver.*;
import dice.Dice;
...
public class Manager implements Manger_Reader {
	 ...
	 * Constructor of GSM
	 */
	public Manager(Player[] p, Vector<Die> dice) {
		myResources = new Mat();
		myGoal = new Goal();
		myForbidden = new Mat();
		myPermitted = new Mat();
...
//		timer = new GameTimer(1);
		...
		for (int i = 0; i < dice.size(); i++) {
			myResources.addToMyMat(dice.elementAt(i));
		}
	}
