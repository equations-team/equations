...
public class GameView implements ActionListener, DropTargetListener{
	...
	private DropTarget pmt;
	private DropTarget req;
	private DropTarget ws;
	private DropTarget gs;
	private EquationsController controller;
	private ImageIcon[] myDieImages;
	
	private Die[] myDice;
	
	public GameView(EquationsController control) {
		controller = control;
		myDieImages = new ImageIcon[24];
		
		for(int i = 0; i < 24; i++) {
			myDieImages[i] = control.getDieImages()[i];
		}
		...
		for(int i = 0; i < dieFaces.length-1; i++) {
			dieFaces[i] = "/myImages/Red1.png";
		}
		dieFaces[dieFaces.length-1] = "/myImages/Red1.png";
		makeDice(dieFaces);
		for(JLabel die : dice) {
			resources.add(die);
		}
		...
		res = new DropTarget(resources, this);
		fbd = new DropTarget(forbidden, this);
		pmt = new DropTarget(permitted, this);
		req = new DropTarget(required, this);
		ws = new DropTarget(workingSolution, this);
		gs = new DropTarget(gameSolution, this);
		...
		for(int i = 0; i < 24; i++) {
			resources.add(new JLabel(myDieImages[i]));
		}
	...
	private void makeDice(String[] dieFaces) {
		...
		for(int i = 0; i < diceImages.length; i++) {
			//diceImages[i] = new ImageIcon(this.getClass().getResource(dieFaces[i]));
			diceImages[i] = myDieImages[i];
	...
	public static void main(String[] args){
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				new GameView(new EquationsController());
			}
		});
	}
}